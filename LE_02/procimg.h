...
void geraImg(unsigned char img[linha][coluna]){
...
//Q11
int L11= 0, C11= 0;  // declaração de variáveis globais para manipular na função
int quantosPixelsNaInt_R (unsigned char img[linha][coluna], unsigned char inte) {
    int count = 0;
    if (L11 >= linha) return 0;          // verifica se todas as linhas estão preenchidas
    if (img [L11][C11]==inte)     // contador de frequência do inte na img
        count++;
    C11++;
    if (C11>=coluna) {        //se completar uma coluna, pula a linha e percorre as colunas dnv
        C11 = 0;
        L11++;
    }
